...
function L:SetupOptionsPanel()
    local category = Settings.RegisterCanvasLayoutCategory(CreateFrame("Frame"), "Mythic Plus Levels & Times")

    local overlayOption = Settings.RegisterAddOnSetting(category, "Enable Overlay", "enableOverlay", MPLTSettings, Settings.VarType.Boolean, "Enable Overlay on Challenges Frame", defaults.enableOverlay)

    Settings.CreateCheckbox(category, overlayOption, "When enabled, overlays dungeon info and teleport buttons on the Blizzard Challenges tab icons.")

    Settings.SetOnValueChanged("enableOverlay", function(_, setting, value)
        L:SaveSetting("enableOverlay", value)
    end)
